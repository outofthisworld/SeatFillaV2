  ...
  createUser: function (req) {
    ...
    return User.create(req.allParams()).then(function create (user) {
      ...
      return UserSettings.create({
        ...
        err.modelErrors = err.modelErrors || []
        err.modelErrors.push({ model: 'User', id: user_partial.user.id })
        err.modelErrors.push({ model: 'Address', id: user_partial.address.id })
        return Promise.reject(err)
      })
      ...
      return UserProfile.create({
        ...
        err.modelErrors.push({ model: 'User', id: user_partial.user.id })
        err.modelErrors.push({ model: 'Address', id: user_partial.address.id })
        err.modelErrors.push({ model: ' UserSettings', id: user_partial.userSettings.id })
        return Promise.reject(err)
      })
      ...
      return Signup.create({
        ...
        err.modelErrors = err.modelErrors || []
        err.modelErrors.push({ model: 'User', id: user_partial.user.id })
        err.modelErrors.push({ model: 'Address', id: user_partial.user.address.id })
        err.modelErrors.push({ model: 'UserSettings', id: user_partial.user.userSettings.id })
        err.modelErrors.push({ model: 'UserProfile', id: user_partial.userProfile.id })
        return Promise.reject(err)
      })
    }).then(function (user_partial) {
      ...
      return EmailService.sendEmailAsync(message).then(function getInfo (info) {
        ...
        err.modelErrors.push({ model: 'Address', id: user_partial.address.id })
        err.modelErrors.push({ model: 'UserSettings', id: user_partial.userSettings.id })
        err.modelErrors.push({ model: 'Signup', id: signup.id })
        err.modelErrors.push({ model: 'UserProfile', id: user_partial.userProfile.id })
            ...
            if (modelError.model in sails.models && '_attributes' in sails.model[modelError.model]) {
              for (var key in sails.models[modelError.model]._attributes) {
                if ('primaryKey' in sails.models[modelError.model]._attributes[key] &&
                  ...
                  sails.model[modelError.model].destroy({ modelPrimaryKey: modelError.id }).then(function () {
      ...
      return new Promise(function (resolve, reject) {
        Address.findOrCreate(addressInfo, addressInfo, function (err, address) {
          if (err) {
            ...
            return resolve({ address, countryInfo: country })
          }
        })
      })
    })
