...
module.exports = function addToCollection(req, res) {
    ...
    var associationAttr = _.findWhere(Model.associations, {
        alias: relation
    });
    ...
    async.auto({
            ...
            parent: function(cb) {
                Model.findOne(parentPk).exec(function foundParent(err, parentRecord) {
                    ...
                    if (!parentRecord) return cb({
                        status: 404
                    });
                    if (!parentRecord[relation]) return cb({
                        status: 404
                    });
        ...
        function readyToSave(err, async_data) {
            ...
            async_data.parent.save(function saved(err) {
                ...
                if (!isDuplicateInsertError && req._sails.hooks.pubsub) {
                    ...
                    if (req.isSocket) {
                        Model.subscribe(req, async_data.parent);
                    }
                    // Publish to subscribed sockets
                    Model.publishAdd(async_data.parent[Model.primaryKey], relation, async_data.actualChildPkValue, !req.options.mirror && req, {
