...
module.exports = {
    ...
    update(req,res){
        ...
        if(req.isGET()){
            CreditCard.find({payer_id:req.user.id})
            .then(function(cards){
                return res.ok({user:req.user,UserProfile,cards},{renderHtml:true});
            })
        }else if(!(req.user.verifyPassword(req.param('password')))){
            req.flash('danger','Invalid password confirmation');
              return res.ok({user:req.user,UserProfile})
        }else{
            req.deleteParam('id');
            req.deleteParam('password');
            req.deleteParam('username');
            User.update({id:req.user.id},req.allParams()).then(function(record){
                ...
                return res.ok({user:record[0],UserProfile});
            }).catch(function(err){
                sails.log.error(err);
    ...
    getCurrentUser(req,res){
        if(!req.user) return res.ok({status:404});
        else return res.ok({status:200,username:req.user.username,id:req.user.id});
    }
};