...
module.exports = {
    ...
    return new Promise(function (resolve, reject) {
      ...
      async.auto({
        findRoutesForEventType(callback) {
          ApiRoutes.find({route: paths}).then(function (routes) {
        ...
        findWebhookForRoutes: ['findRoutesForEventType', function (callback, results) {
          Webhook.find({routes: results.findRoutesForEventType.map(function (route) {return route.id}),isVerified:true})
  ...
  verifyWebhook(options) {
    return new Promise(function (resolve, reject) {
      ...
      if (options.token && options.webHook && options.token == options.webHook.verificationToken) {
        ...
        return Webhook.find({id: options.webHookId})
  ...
  sendWebhookVerification(webHook, url) {
    ...
    return Webhook.update({id: webHook.id}, {verificationToken: token}).then(function (updatedWebhook) {
      return new Promise(function (resolve, reject) {
        if(!updatedWebhook){
        ...
        request({
          ...
          if (err) {
            ...
            return _this.verifyWebhook({webHook: updatedWebhook,token})
          }
          return resolve(updatedWebhook)
        })
      })
