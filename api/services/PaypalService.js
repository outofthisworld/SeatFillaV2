                ...
                function(callback){
                    ...
                    paypal.creditCard.create(data, function(error, credit_card){
                        ...
                        if (error || !credit_card || !credit_card.state == 'OK') {
                            ...
                            CreditCard.findOrCreate({id:credit_card.id},credit_card).then(function(credit_card){
                                CreditCardLinks.create(credit_card.links.map(function(credit_card_link){
                                    ...
                                    return callback(null,{credit_card,credit_card_links});
                                }).catch(function(err){
                                    sails.log.error(err);
                                    CreditCard.destroy({id:credit_card.id})
                                    .then(function(){return callback(err)}).catch(function(error){
                                        sails.log.error(error)
            ...
            async.waterfall([
                function find_credit_card_dets(callback){
                    if(options.credit_card_token && options.credit_card_token.credit_card_id && options.credit_card_token.payer_id){
                        return callback(null,{credit_card_token:options.credit_card_token});      
                    }
                    ...
                    if((options.user || options.payer_id)){
                        queryObj = {payer_id:options.user.id || options.user || options.payer_id, valid_until:{'>':new Date().toISOString()}};
                    }else if(options.credit_card_id){
                        queryObj = {id:options.credit_card_id || options.credit_card.id};
                    }else{
