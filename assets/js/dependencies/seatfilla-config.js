...
window.seatfilla.globals.cache.put = function (options) {
  if (!options || !options.key || !('data'in options)){
...
window.seatfilla.globals.setFixerIoExchangeRates = function(base,data,cb){
  window.seatfilla.globals.cache.put({
    key: 'fixerIoExchangeRates'+base,
    type: 'session',
    data,
...
window.seatfilla.globals.convertCurrency = function(amt,from,to,callback){
    if(from == to){
        return callback(null, amt);
    }
    console.log('recieved ' + JSON.stringify(arguments))
    window.seatfilla.globals.getFixerIoExchangeRates(from,function(err,results){
        if(err || !results || !results.rates || !(to in results.rates)){
            ...
            try{
                ...
                var rate = parseFloat(results.rates[to]);
                console.log(amount);
                console.log(rate)
                console.log(rate*amount)
                return callback(null,rate*amount);
            }catch(err){
...
window.seatfilla.globals.getFixerIoExchangeRates = function (base, callback) {
    window.seatfilla.globals.cache.get({
        key: 'fixerIoExchangeRates'+base,
        type: 'session',
        useServerStore: false,
        success: function (status, result) {
            if(status == 200 && result && result.rates){
                return callback(null,result)
            }else{
                $.ajax({
                    url: '/LookupService/getCurrentCurrencyExchangeRates',
                    method: 'GET',
                    data: {
                    base
                    },
                    success: function (res, ts, xhr) {
                        if (xhr.status == 200 && res.status == 200) {
                            window.seatfilla.globals.setFixerIoExchangeRates(base,res,function(status,eventObj){
                                if(status != 200){
                                    console.log('Error setting fixer io rates');
                                }
                                console.log(eventObj);
                                return callback(null, res)
                            })
                        }else {
                            return callback(new Error('Error retrieving exchange rates'), res)
                        }
                    },
                    error: function (jqXHR, textStatus, errorThrown) {
                        return callback(new Error('Status: ' + jqXHR.status + ' Error retrieving exchange rates, ' + errorThrown), jqXhr.responseJSON)
                    }
                })
            }
        }
          ...
          for (var keyTwo in hotel.images[key]) {
            ...
            if (options) {
              ...
              if (greaterThanOrETSmallestWidth && lessThanOrETGreatestWidth &&
                ...
                arr.push({
                  imagePath: firstPathPart + keyTwo,
                  width,
                height})
            } else {
              arr.push({
...
window.seatfilla.globals.getQParam = function (name, url) {
  if (!url) {
