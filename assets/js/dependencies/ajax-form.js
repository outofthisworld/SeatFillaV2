        ...
        return function(err,res,xhr,ts,errorThrown){
            if(err || res && (res.error || res.errors)){
                if(options && res && options[res.status] && options[res.status] == 'function'){
                     $.toaster({priority:'danger',message: 'Error: ' +  options[res.status].call(null)})
                }else if($.resHasError(res,xhr)){
                     $.toaster({priority:'danger',message: 'Error: ' +  $.stringifyResponseError(res)[0]})
                }else{
                    $.toaster({priority:'danger', message:'Error: ' + (options.errorMessage || '') + ' ' + errorThrown})
                }
            }else{
                if(res && xhr.status == 200 && (!res.errors && !res.error)){
                    $(this).find("input[type=text], textarea").val("")
                    if(options && options.successMessage) $.toaster({priority:'info',message:options.successMessage})
                }else{
                    $.toaster({priority:'info',message:'No error occurred but response status was not 200 or response was empty?'})
                }
            }
        }
    }
