  ...
  */
  ...
  latestNotifications: function(req,res){
    sails.log.debug('attempting to retrieve loatest notifications')
      ...
      NotificationService.findLatestNotifications(req,{limit:req.param('limit'),sort:req.param('sort'),skip:req.param('skip')}).then(function(latestNotifications){
  ...
  sendNotification: function (req, res) {
    NotificationService.sendSystemNotification({message:req.param('message')}).then(function(done){
  ...
  findByUser(req,res){
    async.auto({
      findUser(callback){
        User.findOne({username:req.param('username').toLowerCase()})
        .then(function(user){
      ...
      findNotifications:['findUser',function(callback,results){
        const query = Object.assign({where:{user:results.findUser.id}},req.allParams());
      ...
      findCount:['findUser',function(callback,results){
        Notifications.count({user:results.findUser.id}).exec(function(err,count){
